...
Another example for the return value of a GET method on the resource Filter:
...
GET /filters?entity=user
  ...
  \end{lstlisting}
    ...
    As we can see from the picture belows(BELOW), these modules don't make direct access to the storage: the access is made through a module that handles the connections pool and makes the calls to the native Node.js mySql driver.
...
%   \caption{ This sketch represents the view of one section of the dashboard, the contents fully occupies occupy the page size, exploiting all the space and thus allowing the user to have charts and information larger on the page.}
% \end{figure}
